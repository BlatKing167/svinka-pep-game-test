...
font.init()
font1 = font.SysFont('Ariral', 80)
win = font1.render('YOU WIN!', True, (255, 255, 255))
lose = font1.render('YOU LOSE!', True, (180, 0, 0))

font2 = font.SysFont('Ariral', 36)
...
img_hero = "papaSvin.png" # герой
img_enemy = "jordj.png" # jordj.png
img_ast = "peppa.png" # пеппа
...
def GetBullets(ammo, shotCount):
    return '|' * (ammo - shotCount)
...
class Enemy(GameSprite):
   ...
   def update(self):
       ...
       if self.rect.y > win_height:
           ...
           lost = lost + 1
   def fire_en(self):
       bullet_en = Bullet(img_bullet, self.rect.centerx, self.rect.top, 15, 20, 5)
       bullets_en.add(bullet_en)
...
for i in range(1, 3):
    asteroid = Enemy(img_ast, randint(30, win_width - 30), -40, 100, 100, randint(1, 7))
...
while run:
    ...
    if not finish:
        ...
        if rel_time == True:
            ...
            if now_time - last_time < 2.5: # пока не прошло 3 секунды выводим информацию о перезарядке
        ...
        text_reload = font2.render("Reload: " + str(rel_time) + GetBullets(10, num_fire), 1, (255, 255, 255))
        window.blit(text_reload, (10, 450))
    ...
    #бонус: автоматический перезапуск игры
        ...
        time.delay(3000)
        for i in range(1, 6):
            monster = Enemy(img_enemy, randint(100, win_width - 100), -40, 100, 100, randint(1, 5))
            monsters.add(monster)
        for i in range(1, 3):
            asteroid = Enemy(img_ast, randint(30, win_width - 30), -40, 100, 100, randint(1, 7))
            asteroids.add(asteroid)    
